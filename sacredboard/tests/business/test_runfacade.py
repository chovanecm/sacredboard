...
def a_run_exists(data_access, run_id):
    ...
    assert data_access.get_run_dao().get_run(run_id) is not None
...
def a_metric_of_that_run_exists(data_access, metric_id, run_id):
    ...
    data_access.get_run_dao().get_run(run_id)["info"] = {
        "metrics": [metric_id]}
    ...
    assert data_access.get_metrics_dao().get_metric(run_id,
...
def the_metric_of_that_run_should_not_exist(data_access, metric_id, run_id):
    ...
    with pytest.raises(NotFoundError):
        data_access.get_metrics_dao().get_metric(run_id, metric_id)
...
def data_access():
    ...
    metric_dao.delete_metrics = lambda run_id: metric_dao.should_receive(
        "get_metric").with_args(run_id, object).and_raise(NotFoundError)
    run_dao.delete_run = lambda run_id: run_dao.should_receive(
        "get_run").with_args(run_id).and_raise(NotFoundError)

    # default behaviour:
    run_dao.should_receive("get_run").with_args(object).and_raise(NotFoundError)
    metric_dao.should_receive("get_metric").with_args(object, object).and_raise(
        NotFoundError)
    return data_access
