...
def test_run_tensorboard_other_output():
    with py.raises(p.UnexpectedOutputError) as ex:
        port = p.run_tensorboard("/tmp/logdir", tensorboard_args=["--print-nonsense"])


@py.mark.timeout(15)
def test_run_tensorboard_address_in_use():
    with py.raises(p.UnexpectedOutputError) as ex:
        port = p.run_tensorboard("/tmp/logdir", tensorboard_args=["--address-in-use"])
...
def test_run_tensorboard_timeout():
    with py.raises(TimeoutError) as ex:
        port = p.run_tensorboard("/tmp/logdir", tensorboard_args=["--print-nothing"])
